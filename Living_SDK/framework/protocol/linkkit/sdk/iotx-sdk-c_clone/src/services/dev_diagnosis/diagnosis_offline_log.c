...
static aos_mutex_t s_offline_log_mutex = {NULL};
static bool s_offline_log_mutex_flag = false;
extern int mbedtls_base64_encode(unsigned char *dst, size_t dlen, size_t *olen,
                                 const unsigned char *src, size_t slen);

#define LOCK_RET_ON_ERR() do {\
    if (aos_mutex_lock(&s_offline_log_mutex, AOS_WAIT_FOREVER) != 0)\
    {\
        diagnosis_err("lock fail %d\n", __LINE__);\
        return -1;\
    }\
} while(0)


#define UNLOCK_RET_ON_ERR() do {\
    if (aos_mutex_unlock(&s_offline_log_mutex) != 0)\
    {\
        diagnosis_err("unlock fail %d\n", __LINE__);\
        return -1;\
    }\
} while(0)
...
int diagnosis_offline_log_init(void)
{
    ...
    //Init once, never free!!!
    if (s_offline_log_mutex_flag == false) {
        if ((ret = aos_mutex_new(&s_offline_log_mutex)) != 0)
        {
            diagnosis_err("%s", no_mem_err);
            return ret;
        }
        s_offline_log_mutex_flag = true;
    }
