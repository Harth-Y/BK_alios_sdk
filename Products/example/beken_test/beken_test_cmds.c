...
extern void ping_cmd(char *buf, int len, int argc, char **argv);
...
static void handle_wifistate_cmd(char *pwbuf, int blen, int argc, char **argv)
{
	...
	if (sta_ip_is_start()) {
		...
		case BK_SECURITY_TYPE_EAP:
			...
			break;
		default:
			aos_cli_printf("unknown cipher(%d)\r\n", bk_sta_cipher_type());
			break;
		}
	}
